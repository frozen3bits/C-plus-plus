...
float helpStringToNum(const std::string &text){// 字符串转换为数字
    float ret;
    std::stringstream ss;
    ss << text;
    ss >> ret;
...
void Token::visit(Visitor *visitor)// 遍历默认行为：让访问者看到自己
{
    visitor->apply(this);
};

class NumToken : public Token// 数字标识符
{
    public:
        const float value;
...
class OperatorToken : public Token// 运算符标识符
{
    public:
        const char op;
...
class BracketToken : public Token// 括号标识符
{
    public:
...
class ExprToken : public Token// 表达式标识符
{
    private:
        NumToken *num;
        OperatorToken *op;
...
class PrintVistor : public Visitor// 打印访问者：输出运算过程
{
    private:
        int line;
    public:
...
std::vector<Token *> parse(std::string text)// 词汇分析，从字符串生成基础标识符
{
...
bool is_expr(Token *lhs, Token *op, Token *rhs, bool priority = true)// 判断是否为运算，且优先级与指定优先级相同
{
...
void compile(std::vector<Token *> &vec)// 根据四则运算法则归约标识符
{
